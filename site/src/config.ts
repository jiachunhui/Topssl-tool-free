...
export const FAQS = [
  ...
  {
    q: '换电脑后要重新配置一遍吗？',
    a: '不用。在「设置 → 数据备份与迁移」中把证书、设置、DNS 凭据与 ACME 账户密钥导出为一个口令加密的备份文件，在新电脑导入即可继续使用，无需重新申请证书。注意导入后自动续期会被自动关闭，请只在一台常开的电脑上重新开启（两台同时续期会浪费签发配额并可能互相干扰 DNS 校验）。详见换电脑迁移教程。',
  },
  {
    q: '备份包安全吗？口令忘了怎么办？',
    a: '备份包使用 AES-256-GCM + PBKDF2-HMAC-SHA256（60 万次迭代）口令加密，内容不会上传到任何服务器。口令无法找回，请把备份文件与口令一起妥善保管；文件内含证书私钥与 DNS API 密钥，不要通过不可信的渠道传输，迁移完成后建议及时删除临时备份文件。',
  },
  {
    q: '遇到问题如何获得支持？',
